...
void	raycasting(t_game *cub)
{
	...
	cub->ray.ray_dir_x = 0;
	cub->ray.ray_dir_y = 0;
	cub->ray.dir_x = 0; // depend on player pos -- here is for NORTH
	cub->ray.dir_y = -1; // depend on player pos -- here is for NORTH
	cub->ray.plane_x = 0.66; // depend on player pos -- here is for NORTH
	cub->ray.plane_y = 0; // depend on player pos -- here is for NORTH
	/*DELTA DISTANCE CALCULATION*/
	cub->ray.pos_x = 5.5; //position_player_x;
	cub->ray.pos_y = 2.5; //position_player_y;
	cub->ray.map_x = (int)cub->ray.pos_x; 
	cub->ray.map_y = (int)cub->ray.pos_y; 
	cub->ray.delta_dist_x = 0;
	cub->ray.delta_dist_y = 0;
	/*MOVING THE RAY*/
	cub->ray.step_x = 0; // step_x = -1 -->left // step_x = 1 -->right
	cub->ray.step_y = 0; // step_y = -1 -->up // step_y = 1 -->down
	cub->ray.side_dist_x = 0; //store dist from ray pos to next x or y grid line
