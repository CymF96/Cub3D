...
{
	if (cub->player_pos != 1)
		ft_exit("Information about player position is incorrect", cub);
...
void	readmap(t_game *cub, int fd)
{
	...
	if (line == NULL)
		ft_exit("reading the map failed", cub);
	while (line && line[0] != '\0')
	{
		nbr_line++;
...
bool	map_validation(t_game *cub, char *input)
{
	...
	if (!map_format(input))
		ft_exit("Map format isn't correct. \
			Please use .cub only", NULL);
	fd = open(input, O_RDONLY);
	if (fd < 0)
		ft_exit("Map file can't be read. \
			Please check the map filename", NULL);
	readmap(cub, fd);
	check_map_rules(cub);
	return (true);
}
