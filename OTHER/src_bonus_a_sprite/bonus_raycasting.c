...
/*   Updated: 2024/12/06 10:33:54 by cofische         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void render_wall(t_game *game, t_ray *ray, int x)
{
	...
	{
		map_x = (int)(ray->ray_x / BLOCK_SIZE);
        map_y = (int)(ray->ray_y / BLOCK_SIZE);														 // cheking the ray assignement
		ray->tex_y = (int)pos % BLOCK_SIZE;
        if (game->map[map_y][map_x] == 'D' && game->d.timer != 1.0)
			render_door(game, &game->d, x);
		else
		{
